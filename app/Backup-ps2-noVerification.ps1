...
function Get-ArchiveItems_ByDateGroups {
    ...
    foreach ($group in $grouped) {
        $dateStr = $group.Name  # "20260330"
        
        # SourceName = имя группы для лога (как $fld.Name)
        $groupName = "DateGroup_$dateStr"
        
        $archiveName = Resolve-ArchivePattern -Pattern $pattern -PCName $PCName -JobName $JobName -LastWriteDate $dateStr
        
        # **КРИТИЧНО**: SourcePath = ПЕРВЫЙ файл группы (как $fld.FullName)
        # Остальные файлы передаются в основной код через SourceFilter или отдельно
        $firstFile = $group.Group[0].FullName
        
        $items += (New-Object PSObject -Property @{
            SourcePath   = $firstFile  # ✅ Один путь, как в IndividualFolders
            SourceName   = $groupName  # "DateGroup_20260330" — для лога
            ArchiveName  = $archiveName  # "PC001_Job_20260330.rar"
            SourceType   = 'DateGroupFiles'
            SourceRoot   = $Job['Source']
            SourceFilter = $filter     # Фильтр для получения всех файлов группы
            GroupDate    = $dateStr    # Доп. свойство для основного кода
        })
    }
    return $items
}
