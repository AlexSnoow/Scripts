...
.EXAMPLE
    powershell.exe -executionpolicy RemoteSigned -file .\Backup-rar-ps2-dp.ps1
...
$xmlPath = Join-Path $ScriptRoot "Backup-Config-All.xml"
if (-not (Test-Path $xmlPath)) {
    $parentDir = Split-Path $ScriptRoot -Parent
    $xmlPath = Join-Path $parentDir "common\Backup-Config-All.xml"
}
if (-not (Test-Path $xmlPath)) {
