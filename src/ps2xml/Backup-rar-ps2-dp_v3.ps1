<#
.SYNOPSIS
    File Backup-rar-ps2-dp_v1.ps1
    Автономный скрипт резервного копирования (Версия 2.5 RAR-only)
.DESCRIPTION
    Единый файл, включающий все функции модулей (только RAR):
...
.EXAMPLE
    powershell.exe -executionpolicy RemoteSigned -file .\Backup-rar-ps2-dp_v1.ps1

.NOTES
    Версия: 2.5 RAR
    Дата: 2026-03-19
    Совместимость: PowerShell 2.0 и выше
#>
...
foreach ($jobNode in $b.Jobs.Job) {
    ...
    # ExcludeFilePattern (пока сохраняем, позже добавим логику)
...
# ===========================================================
#region ФУНКЦИЯ ВЫЧИСЛЕНИЯ SHA256 ХЕША (совместимость с PowerShell 2)
# ===========================================================
...
function Start-RarArchive {
    ...
    process {
        ...
        try {
            ...
            # В PS 2.0 свойства StandardOutputEncoding нет, но кодировка OEM по умолчанию подходит
...
function Get-FileList {
    ...
    param(
        ...
        [ValidateScript({ if (-not (Test-Path -LiteralPath $_ -PathType Container)) { throw "Директория не существует: $_" } $true })]
        [string]$Path
    )
    begin {
...
function Get-DiskSpaceReport {
    ...
    # Для удалённых компьютеров этот метод работает только локально.
    # Если нужна поддержка удалённых ПК, потребуется использовать WMI или WinRM.
    # В вашем сценарии, скорее всего, достаточно локального отчёта.
    try {
...
foreach ($jobName in $config['Jobs'].Keys) {
    ...
    try {
        ...
        if ($archiveIndividual) {
			...
			#region Формирование XML-отчёта
            ...
            # Суммарный размер успешных архивов
            ...
            #endregion /Формирование XML-отчёта
			
            continue
        }

		#endregion /Блок Архивации файлов в отдельный архив
        ...
        if ($archiveIndividualFolders) {
            ...
            $folderFilter = if ($job.ContainsKey('SourceFilter') -and -not [string]::IsNullOrWhiteSpace($job['SourceFilter'])) { $job['SourceFilter'] } else { $null }
            $excludeFolderPattern = if ($job.ContainsKey('ExcludeFolderPattern') -and -not [string]::IsNullOrWhiteSpace($job['ExcludeFolderPattern'])) { $job['ExcludeFolderPattern'] } else { $null }
		...
		#endregion /Блок Архивации каталогов в отдельный архив
        ...
        $sourceFilter = if ($job.ContainsKey('SourceFilter') -and -not [string]::IsNullOrWhiteSpace($job['SourceFilter'])) { $job['SourceFilter'] } else { $null }
...
if (Test-Path $logDir) {
    $allLogs = Get-ChildItem -Path $logDir -File | Sort-Object LastWriteTime -Descending
