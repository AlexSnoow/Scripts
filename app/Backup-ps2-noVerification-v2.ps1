<#
.SYNOPSIS
    File Backup-ps2-v3.ps1
    Автономный скрипт резервного копирования (Версия 3.2 Unified Pipeline)
.DESCRIPTION
...
.NOTES
    Версия: 2 (Архивы по дате создания создаются но 1 файл в архиве)
    Дата: 2026-04-12
...
$XmlFile = "Backup-Config-All-test.xml"
$XmlHash = "D3A5474B30E04DA091189FAC2726BFD03A83FE1C5C9757F6B4783A36EB1C9AC0"
...
# ==============================================================================
#region ЭТАП 0: ПРОВЕРКА XML наличие и Hash
# ==============================================================================
...
foreach ($jobNode in $b.Jobs.Job) {
    ...
    $Script:EmbeddedConfig['Jobs'][$jn] = @{
		...
		# ArchivePattern           = if ($jobNode.ArchiveByDate) { $Script:EmbeddedConfig['Jobs'][$jn]['ArchiveByDatePattern'] = $jobNode.ArchiveByDatePattern } else { $jobNode.ArchivePattern }
	...
	if ($jobNode.ArchiveByDate) {
        $Script:EmbeddedConfig['Jobs'][$jn]['ArchiveByDate'] = $jobNode.ArchiveByDate
    }
	if ($jobNode.ExcludeTodayFiles) {
        $Script:EmbeddedConfig['Jobs'][$jn]['ExcludeTodayFiles'] = $jobNode.ExcludeTodayFiles
    }
}
...
function Start-RarArchive {
    ...
    param(
        ...
        [Parameter(Mandatory=$true)][object]$SourcePath,  # ✅ object — строка ИЛИ массив
        [Parameter(Mandatory=$false)][string[]]$Parameters = @("a","-m3","-ep1","-rr3p","-r","-dh","-t"),
    ...
    begin {
        ...
        # ✅ ЛОГИКА ДЛЯ МАССИВА ФАЙЛОВ (DateGroupFiles)
        $safeSourcePaths = @()
        if ($SourcePath -is [array] -or $SourcePath -is [System.Collections.ArrayList]) {
            # Массив путей: rar.exe a archive.rar "file1" "file2" ...
            foreach ($src in $SourcePath) {
                $safeSourcePaths += '"' + ($src.ToString() -replace '"', '\\"') + '"'
            }
        } elseif (-not (Test-StringIsNullOrWhiteSpace($SourceFilter))) {
            # Директория + фильтр
            $filteredPath = Join-Path -Path $SourcePath -ChildPath $SourceFilter
            $safeSourcePaths += '"' + ($filteredPath -replace '"', '\\"') + '"'
        } else {
            # Один путь
            $safeSourcePaths += '"' + ($SourcePath.ToString() -replace '"', '\\"') + '"'
        }
        
        $argsList += @($safeArchivePath) + $safeSourcePaths
    }
    
    process {
        $processStart = Get-Date
        try {
            Write-Verbose "RAR команда: $RarPath $($argsList -join ' ')"
...
# ============================================================
#region UNIFIED PIPELINE: Формирование элементов архивации
# ============================================================
...
function Resolve-ArchivePattern {
	...
	$name = $name -replace '{LastWriteDate}', $LastWriteDate
...
function Get-ArchiveItems_ByDateGroups {
    [OutputType([object[]])]
    param(
        [hashtable]$Job,
        [string]$PCName,
    ...
    Write-Host "=== ENTER ByDateGroups ===" -ForegroundColor Yellow
    $filter = $Job['SourceFilter']
    if (Test-StringIsNullOrWhiteSpace($filter)) {
        throw "Для архивации по датам требуется SourceFilter"
    }
    ...
    $today = (Get-Date).Date
    $files = $files | Where-Object { (Get-Item $_.FullName).LastWriteTime.Date -lt $today }
	Write-Log "Files AFTER today filter: $($files.Count)" -Level INFO
    if ($files.Count -eq 0) {
        Write-Log "Файлы по маске '$filter' со старыми датами не найдены" -Level WARNING
        return @()
    }
    
    # Группируем по дате (строка yyyyMMdd)
    $grouped = $files | Group-Object { (Get-Item $_.FullName).LastWriteTime.Date.ToString("yyyyMMdd") }
    Write-Log "Подготовлено $($grouped.Count) групп дат, файлов: $($files.Count)" -Level INFO
	
    $pattern = "{PCName}_{JobName}_{LastWriteDate}.rar"
    if ($Job.ContainsKey('DateGroupArchivePattern') -and -not (Test-StringIsNullOrWhiteSpace($Job['DateGroupArchivePattern']))) {
        $pattern = $Job['DateGroupArchivePattern']
    }
    
    $items = @()
    foreach ($group in $grouped) {
		Write-Host "PROCESS GROUP: $($group.Name), files: $($group.Group.Count)" -ForegroundColor Yellow
        Write-Log "  SourcePath type: $($group.Group.FullName.GetType().Name)" -Level SUCCESS
		Write-Log "  First file: $($group.Group[0].FullName)" -Level SUCCESS
        $dateStr = $group.Name
        
        $archiveName = Resolve-ArchivePattern -Pattern $pattern -PCName $PCName -JobName $JobName -LastWriteDate $dateStr
		Write-Log "Имя архива : $($archiveName)" -Level SUCCESS
        # ✅ МАССИВ полных путей всех файлов группы (НЕ строка!)
        $allFilesInGroup = $group.Group.FullName  # PSObject[].FullName → массив строк
        
        $items += (New-Object PSObject -Property @{
			SourcePath   = $group.Group.FullName  # ✅ МАССИВ строк путей!
			SourceName   = "DateGroup_$dateStr"
			ArchiveName  = $archiveName
			SourceType   = 'DateGroupFiles'
			SourceRoot   = $Job['Source']
			SourceFilter = $null  # ✅ НЕ используем фильтр
		})
    }
    Write-Log "RETURN ITEMS: $($items.Count)" -Level SUCCESS
    return $items
}
...
function Get-ArchiveMode {
	...
	if ($Job.ContainsKey('ArchiveByDate')) { $indGroupByDate = [System.Convert]::ToBoolean($Job['ArchiveByDate']) }
    if ($indFiles)   { return 'IndividualFiles' }
    if ($indFolders) { return 'IndividualFolders' }
	if ($indGroupByDate) { return 'IndividualGroupByDate' }
    return 'Normal'
}
...
function Prepare-ArchiveItems {
    ...
    switch ($mode) {
		...
		'IndividualGroupByDate' { return Get-ArchiveItems_ByDateGroups -Job $Job -PCName $PCName -JobName $Job['Name'] }
...
function Invoke-ArchivePipeline {
    ...
    foreach ($item in $ArchiveItems) {
		...
		# Логика для DateGroupFiles — передаем ВСЕ файлы группы
		$sourcePathsToArchive = $item.SourcePath
		if ($item.SourceType -eq 'DateGroupFiles') {
			# SourcePath уже массив путей — используем как есть
			$sourcePathsToArchive = $item.SourcePath
			Write-Log "Архивация группы $($item.SourceName): $($sourcePathsToArchive.Count) файлов" -Level INFO
		}
		...
		$arhResult = Start-RarArchive `
			...
			-SourcePath $sourcePathsToArchive `  # ✅ Может быть массивом!
...
# function Invoke-Verification {
    # [OutputType([hashtable])]
    # param(
        # [object[]]$ArchiveResults,  # успешные результаты из Invoke-ArchivePipeline
        # [hashtable]$Job,
        # [hashtable]$Config
    # )
    # $rarPath = $Config['Settings']['ArchiverPath']
    ...
    # foreach ($res in $ArchiveResults) {
        # if ($res.Status -ne 'Success' -or -not (Test-Path -LiteralPath $res.ArchivePath -PathType Leaf)) {
            # Write-Log "Пропуск верификации (архив не создан): $($res.SourceName)" -Level WARNING -ResultKey
            # continue
        # }
        ...
        # try {
            ...
            # if ($res.SourceType -eq 'File') {
                # Один файл
                # $fi = Get-Item -LiteralPath $res.SourcePath -ErrorAction Stop
                # $sourceFiles += (New-Object PSObject -Property @{
                    # RelativePath  = $res.SourceName
                    # Length        = $fi.Length
                    # LastWriteTime = $fi.LastWriteTime
                    # Source        = "FileSystem"
                    # FullName      = $fi.FullName
                # })
                # Write-Log "Верификация файла: $($res.SourceName) (размер: $($fi.Length) байт)" -Level INFO
            # }
            # elseif ($res.SourceType -eq 'Folder') {
                # Папка — сканируем рекурсивно
                # $sourceFiles = Get-FileList -Path $res.SourceRoot
                # Write-Log "Верификация папки: $($res.SourceName) ($($sourceFiles.Count) файлов, включая 0-байтовые)" -Level INFO
                # if ($sourceFiles.Count -eq 0) {
                    # Write-Log "Папка '$($res.SourceName)' пуста — верификация пропущена" -Level WARNING -ResultKey
                    # $verified += $res
                    # continue
                # }
            # }
            # else {
                # Directory (Normal mode) — весь источник
                # if ($res.SourceRoot) {
                    # $sourceFiles = Get-FileList -Path $res.SourceRoot
                    # Write-Log "Верификация источника: $($res.SourceName) ($($sourceFiles.Count) файлов, включая 0-байтовые)" -Level INFO
                    # if ($sourceFiles.Count -eq 0) {
                        # Write-Log "Источник '$($res.SourceName)' пуст — верификация пропущена" -Level WARNING -ResultKey
                        # $verified += $res
                        # continue
                    # }
                # }
            ...
            # $archiveFiles = Get-FileArhListRar -RarPath $rarPath -ArchivePath $res.ArchivePath
            # if (-not ($archiveFiles) -or $archiveFiles.Count -eq 0) {
                # Если source тоже пуст — считаем совпадением
                # if ($sourceFiles.Count -eq 0) {
                    # Write-Log "Обе стороны пусты — верификация OK: $($res.SourceName)" -Level SUCCESS -ResultKey
                    # $verified += $res
                    # continue
                # }
                # Write-Log "Архив пуст, но источник содержит $($sourceFiles.Count) файлов!" -Level ERROR -ResultKey
                # $failed++
                # continue
            # }

            # === СРАВНЕНИЕ (включая файлы 0 байт — Length == 0 сравнивается корректно) ===
            # $verifyResult = Compare-FilesSourceArchive `
                # -SourceList $sourceFiles `
                # -ArchiveList $archiveFiles `
                # -SourcePath $res.SourceRoot

            # if ($verifyResult.IsIdentical) {
                # Write-Log "ВЕРИФИКАЦИЯ ОК: $($res.SourceName) ($($sourceFiles.Count) файлов, включая 0-байтовые)" -Level SUCCESS -ResultKey
                # $verified += $res
            # } else {
                # Write-Log "ВЕРИФИКАЦИЯ ПРОВАЛЕНА: $($res.SourceName) — $($verifyResult.Report)" -Level ERROR -ResultKey
                # $failed++
            # }
        # }
        # catch {
            # Write-Log "Ошибка верификации $($res.SourceName): $_" -Level ERROR -ResultKey
            # $failed++
        # }
    # }
    ...
    # return @{
        ...
        # AllPassed      = ($failed -eq 0) -and ($verified.Count -eq ($ArchiveResults | Where-Object { $_.Status -eq 'Success' }).Count)
    # }
# }
...
function Invoke-PostOperations {
    param(
        ...
        # [int]$PipelineSuccessCountPipelineSuccessCount,
    ...
    if ($Job['RemoteDest'] -and (Test-Path $Job['RemoteDest'])) {
        ...
        #region Ротация удалённого хранилища
    ...
    #region!!!=== 3. Удаление источника после верификации ===
...
foreach ($jobName in $config['Jobs'].Keys) {
    ...
    try {
        ...
        Save-RemoteReports -PCName $PCName -JobName $jobName -JobStatus $js `
            ...
            -ArchiveSizeMB $totalArchiveSize -LocalLogPath $(Get-LogFilePath) -SourceFileList $sourceFileList
