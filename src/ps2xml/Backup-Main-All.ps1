...
# ===========================================================
#region CONFIG_BLOCK
# Этот блок заменяется build.ps1 на встроенную конфигурацию.
# Для разработки: раскомментируйте загрузку XML ниже.

# region FALLBACK: Если EmbeddedConfig не установлен — загрузить из XML
if (-not (Test-Path variable:Script:EmbeddedConfig) -or $null -eq $Script:EmbeddedConfig) {
    ...
    if (Test-Path $xmlPath) {
        ...
        $Script:EmbeddedConfig = @{
            ...
            Paths = @{
                ...
                SevenZipPath = $b.Paths.SevenZipPath
            }
            Recipients = @{
            ...
            Integrity = @{
                ...
                SevenZipExeHash = $b.Integrity.SevenZipExeHash
            }
            Jobs = @{}
        }
        foreach ($jobNode in $b.Jobs.Job) {
            ...
            if ($jobNode.ArhParameters) {
                $ap = @(); foreach ($p in $jobNode.ArhParameters.Param) { $ap += $p }
                $Script:EmbeddedConfig.Jobs[$jn]['ArhParameters'] = $ap
            }
            if ($jobNode.SourceCheckMasks) {
                $sm = @(); foreach ($m in $jobNode.SourceCheckMasks.Mask) { $sm += $m }
...
# Алисовый слой совместимости: $config.Settings → $BackupConfig.*
$config = @{
    Settings = @{
        ...
        ArchiverPath   = $null  # заполняется в ЭТАП 1
        ArchiverHash   = $null  # заполняется в ЭТАП 1
...
# ===========================================================
# Извлечение базовых переменных (конфиг уже загружен в CONFIG_BLOCK)
# ===========================================================
$PCName = $env:COMPUTERNAME
...
switch ($archiverType) {
    "RAR" {
        if ($BackupConfig.ContainsKey('Paths') -and $BackupConfig.Paths.ContainsKey('RarPath')) {
            $archiverPathValue = $BackupConfig.Paths.RarPath
            $config.Settings.ArchiverPath = $archiverPathValue
        }
        if ($BackupConfig.ContainsKey('Integrity') -and $BackupConfig.Integrity.ContainsKey('RarExeHash')) {
            $archiverHash = $BackupConfig.Integrity.RarExeHash
            $config.Settings.ArchiverHash = $archiverHash
        }
        $archiverFileType = "RAR.exe"
    }
    "7Z" {
        if ($BackupConfig.ContainsKey('Paths') -and $BackupConfig.Paths.ContainsKey('SevenZipPath')) {
            $archiverPathValue = $BackupConfig.Paths.SevenZipPath
            $config.Settings.ArchiverPath = $archiverPathValue
        }
        if ($BackupConfig.ContainsKey('Integrity') -and $BackupConfig.Integrity.ContainsKey('SevenZipExeHash')) {
    ...
    "ZIP" {
        ...
        $config.Settings.ArchiverPath = "builtin"
...
function Write-WinEventAppLog {
    ...
    try {
        ...
        if (-not [System.Diagnostics.EventLog]::SourceExists($Source)) {
            Write-Log "EventLog: источник '$Source' не зарегистрирован. Запись в журнал невозможна." -Level WARNING
            Write-Log "Для регистрации выполните от имени Администратора:" -Level WARNING
            Write-Log "  New-EventLog -LogName '$LogName' -Source '$Source'" -Level WARNING
            return
        }
...
<#
.SYNOPSIS
    Запись XML-отчёта о результате выполнения задания.
.DESCRIPTION
    Создаёт XML-файл отчёта для каждого задания и обновляет сводный файл хоста.
    Используется для централизованного мониторинга через GUI Manager.
.PARAMETER JobName
    Имя задания
.PARAMETER JobStatus
...
.PARAMETER ArchiveSizeMB
    Размер архива в МБ
.PARAMETER Verification
    Результат верификации: Passed, Failed, Skipped
.PARAMETER Errors
    Массив ошибок
.PARAMETER Warnings
...
function Write-BackupXmlReport {
    ...
    process {
        try {
            ...
            # Генерация XML через StringBuilder (PS 2.0 совместимо)
...
<#
.SYNOPSIS
    Запись сводного XML-отчёта хоста.
.DESCRIPTION
    Обновляет файл {Host}_summary.xml с результатами всех заданий.
#>
...
function Get-BackupConfiguration {
    ...
    switch ($archiverType) {
        "RAR" {
            if ($LocalConfig.ContainsKey('Paths') -and $LocalConfig.Paths.ContainsKey('RarPath')) {
                $archiverPathValue = $LocalConfig.Paths.RarPath
            }
        }
        "7Z" {
            if ($LocalConfig.ContainsKey('Paths') -and $LocalConfig.Paths.ContainsKey('SevenZipPath')) {
...
foreach ($jobName in $config.Jobs.Keys) {
    ...
    try {
        ...
        if ($archiveIndividual) {
            ...
            # === БЕЗОПАСНОЕ УДАЛЕНИЕ: Верификация каждого архива перед удалением источника ===
            if ($job.RemoveSourceFlag) {
                ...
                foreach ($archiveResult in $individualResults) {
                    ...
                    try {
                        # Получаем исходный файл и содержимое архива
                        ...
                        $archiveFiles = Get-FileArhList -ArchiverType $config.Settings.ArchiverType -ArchiverPath $config.Settings.ArchiverPath -ArchivePath $archiveResult.ArchivePath
                ...
                if ($verificationFailed -gt 0) {
                    ...
                    # Удаляем ТОЛЬКО файлы, прошедшие верификацию
                    foreach ($item in $verifiedForDeletion) {
                        $fullPath = if ($item.PSObject.Properties.Name -contains 'SourceFileFullName') { $item.SourceFileFullName } else { Join-Path $job.Source $item.SourceFile }
        ...
        if ($archiveIndividualFolders) {
            ...
            # === БЕЗОПАСНОЕ УДАЛЕНИЕ: Верификация каждого архива перед удалением папок источника ===
            if ($job.RemoveSourceFlag) {
                ...
                if ($verificationFailed -gt 0) {
                    ...
                    # Удаляем ТОЛЬКО папки, прошедшие верификацию
                    foreach ($item in $verifiedFoldersForDeletion) {
                        $folderPath = Join-Path $job.Source $item.SourceFolder
                        # ВАЖНО: Не удаляем папку с текущей датой (исключение уже применено при архивации)
