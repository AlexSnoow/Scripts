...
.NOTES
    Версия: 5.3 (добавление функций по созданию архивов по дате последнего изменения файлов)
    Дата: 2026-04-21
...
foreach ($jobNode in $b.Jobs.Job) {
    ...
    if ($jobNode.ArchiveByDatePattern) { 
        $Script:EmbeddedConfig['Jobs'][$jn]['ArchiveByDatePattern'] = $jobNode.ArchiveByDatePattern 
    }
...
function Get-ArchiveItems_ByDate {
    <#
    .SYNOPSIS
    Группирует файлы по дате LastWriteTime для архивации по дням.
    .DESCRIPTION
    Сканирует источник, группирует файлы по дате модификации (формат YYYYMMDD),
    исключает файлы за текущую дату (если задано), создаёт временные list-файлы
    для передачи в RAR через @listfile.
    .PARAMETER Job
    Конфигурация задания из XML.
    .PARAMETER PCName
    Имя компьютера.
    .PARAMETER JobName
    Имя задания.
    ...
    .LINK
    https://internal/wiki/backup-bydate
    #>
    [CmdletBinding()]
    [OutputType([object[]])]
    param(
    ...
    process {
        ...
        $excludeToday = $true
        if ($Job.ContainsKey('ExcludeTodayFiles')) {
            $excludeToday = [System.Convert]::ToBoolean($Job['ExcludeTodayFiles'])
        }

        $filter = if ($Job.ContainsKey('FileFilter') -and -not (Test-StringIsNullOrWhiteSpace($Job['FileFilter']))) {
            $Job['FileFilter']
        }
        elseif ($Job.ContainsKey('SourceFilter') -and -not (Test-StringIsNullOrWhiteSpace($Job['SourceFilter']))) {
            $Job['SourceFilter']
        }
        else { '*.*' }
        ...
        # Группировка через hashtable (PS 2.0 совместимо)
        $groups = @{}
        foreach ($f in $allFiles) {
            $dateStr = $f.LastWriteTime.ToString('yyyyMMdd')
            if ($excludeToday -and $dateStr -eq $todayStr) { continue }
            if (-not $groups.ContainsKey($dateStr)) {
                $groups[$dateStr] = New-Object System.Collections.ArrayList
            }
        ...
        if (-not (Test-Path $tempListDir)) {
            New-Item -Path $tempListDir -ItemType Directory -Force | Out-Null
        }
        ...
        foreach ($dateKey in $groups.Keys) {
            ...
            # Создаём listfile для RAR (OEM кодировка)
